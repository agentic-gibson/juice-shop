...
void describe('/redirect', () => {
  void it('GET redirected to https://github.com/ExploitHunterApp/hard-juice-stop when this URL is passed as "to" parameter', async () => {
    const res = await request(app)
      .get('/redirect?to=https://github.com/ExploitHunterApp/hard-juice-stop')
      .redirects(0)
  ...
  void it('GET redirected to https://www.stickeryou.com/products/hard-juice-stop/794 when this URL is passed as "to" parameter', async () => {
    const res = await request(app)
      .get('/redirect?to=https://www.stickeryou.com/products/hard-juice-stop/794')
      .redirects(0)
    assert.equal(res.status, 302)
  })
  ...
  void it('GET redirected to target URL in "to" parameter when a allow-listed URL is part of the query string', async () => {
    const res = await request(app)
      .get('/redirect?to=/score-board?satisfyIndexOf=https://github.com/ExploitHunterApp/hard-juice-stop')
      .redirects(1)
